...
export class CardTypeRepository {
    ...
    private async triggerEvent(data: CardTypeEntityEvent | CardTypeUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("dirigible-bank-server-Settings-CardType", ["trigger"]);
        ...
        producer.topic("dirigible-bank-server-Settings-CardType").send(JSON.stringify(data));
    }
}
