...
export class BankAccountStatusRepository {
    ...
    private async triggerEvent(data: BankAccountStatusEntityEvent | BankAccountStatusUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("dirigible-bank-server-Settings-BankAccountStatus", ["trigger"]);
        ...
        producer.topic("dirigible-bank-server-Settings-BankAccountStatus").send(JSON.stringify(data));
    }
}
