...
const dao = require("dirigible-bank-server/gen/dao/entities/BankAccountStatus");
const http = require("dirigible-bank-server/gen/api/utils/http");
...
rs.service()
	...
	.resource("{id}")
		.get(function(ctx) {
			...
			if (entity) {
				...
				http.sendResponseNotFound("BankAccountStatus not found");
			}
		})
		.produces(["application/json"])
		.catch(function(ctx, error) {
	...
	.resource("")
		.post(function(ctx, request, response) {
			...
			response.setHeader("Content-Location", "/services/js/dirigible-bank-server/gen/api/BankAccountStatus.js/" + entity.Id);
